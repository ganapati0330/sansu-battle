...
小学1〜3年生の算数を、フテ猫・クロネコさん・チャッピーちゃんと一緒にバトル形式で練習できるブラウザゲームです。
...
1. べんきょうするメニューを えらぶ。選択画面は上から **ぜんぶミックス → 分野べつ → ふくしゅう → むずかしさ** の順
   （「ぜんぶミックス」が初期選択。10ジャンルからランダムに出るので、まずはこれがおすすめ）
2. 全10問。正かいするとクロネコさんに攻撃、まちがえるとフテ猫がダメージ
3. はやく答えるほど、また連続で正かいするほどダメージが大きい
4. さいごに正かい数・正かいりつ・まちがえた問題のふりかえりが出る
5. まちがえた問題は自動で記録され、あとから「ふくしゅう」で解きなおせる
6. 「にがて分析」でどの分野が弱いかを確認できる

チャッピーちゃんのヒントは1回のバトルで3回まで使えます。

## フテ猫の体力とチャッピーちゃんの回復

下のゲージがフテ猫の体力です（上はクロネコさん）。

- まちがえると12ダメージ（★★★は14）、時間ぎれは10ダメージ
- 体力が**35%以下**になると、チャッピーちゃんが自動で**30回復**してくれます
- 回復は1回のバトルにつき**2回**まで。残り回数はゲージ横の⚡マークで確認できます
- 回復すると効果音とともにゲージが緑に光り、クロネコさんが「ちっ、かいふくかよ！」と悔しがります

回復が残っているうちは体力が0になりません。何回かまちがえてもバトルを最後まで続けられるので、途中で心が折れずに10問やりきれます。結果画面には回復を使った回数も出ます。

バトル中はクロネコさんがしゃべります。ダメージを受けたとき、こちらがまちがえたとき、
体力が3割を切って「ライバルモード」になったとき、勝ったとき／負けたときで内容が変わります。
...
- **こえ** はブラウザの音声合成（Web Speech API）でクロネコさんのセリフを読み上げます。
...
GitHub Pages などに置いてブラウザで開いたあと、ホーム画面に追加するとアプリのように起動できます。
...
更新を配布するときは `sw.js` の `VERSION` を上げてください（`'v15'` → `'v16'` など）。古いキャッシュは自動で消えます。
...
GitHub Pages に置く場合は、リポジトリのルート（または `docs/`）にこのフォルダの中身をそのまま入れてください。`.nojekyll` があるので追加の設定は不要です。
...
公開先は `https://ganapati0330.github.io/futeneko-sansu-battle/` になります。
